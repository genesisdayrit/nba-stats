...
def insert_player_stats_to_db(game_date):
    ...
    if len(games) > 0:
        for game in games:
            ...
            headers = player_stats['headers']  # This gives us the field names
            data = player_stats['data']

            for player in data:
                # Build a dictionary mapping API field names to corresponding values
                ...
                db_player_data = {
                    ...
                    'stl': player_data['STL'],
                    'blk': player_data['BLK'],
                    'tos': player_data['TO'],  # Mapped to 'tos'
                    'pf': player_data['PF'],
                    'pts': player_data['PTS'],
